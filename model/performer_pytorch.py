# Source of code reference: https://github.com/TencentAILabHealthcare/scBERT
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, output_attentions = False, **kwargs):
        ...
        if not empty(q):
            if exists(context_mask):
                global_mask = context_mask.unsqueeze(-1)
...
def get_pos_pad_mask(seq_q):
    batch_size, seq_len = seq_q.size()

    pad_attn_mask = ~seq_q.data.eq(0)
    return pad_attn_mask

def get_final_pos_emb(position_matrix,mask_matrix ):
    expanded_mask = mask_matrix.unsqueeze(-1)
    batch_position_matrix = position_matrix.unsqueeze(0).expand(mask_matrix.size(0), -1, -1)
    masked_positions = batch_position_matrix * expanded_mask
...
class PerformerLM(nn.Module):
    def __init__(
        self,
        num_tokens=23,                         # num of tokens
        V_Beta_size=52,
        D_Beta_size=3,
        J_Beta_size=14,
        max_seq_len=26,                        # max length of sequence
        dim=512,                                # dim of tokens
        depth=12,                              # layers
        heads=8,                              # num of heads
        ...
        self.V_Beta = nn.Embedding(V_Beta_size, dim)
        self.D_Beta = nn.Embedding(D_Beta_size, dim)
        self.J_Beta = nn.Embedding(J_Beta_size, dim)
    ...
    def forward(self, x, input_V_Beta,input_D_Beta,input_J_Beta, return_encodings = False, output_attentions = False, **kwargs):
    #def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        b, n, device = *x.shape, x.device
        assert n <= self.max_seq_len, f'sequence length {n} must be less than the max sequence length {self.max_seq_len}'
        # token and positional embedding

        enc_self_attn_mask = get_attn_pad_mask(x)
        enc_pos_mask = get_pos_pad_mask(x)
        
        x = self.token_emb(x)

        token_embedding_V_Beta=self.V_Beta(input_V_Beta)
        token_embedding_D_Beta=self.D_Beta(input_D_Beta)
        token_embedding_J_Beta=self.J_Beta(input_J_Beta)
        
        x +=token_embedding_V_Beta.unsqueeze(1)
        x +=token_embedding_D_Beta.unsqueeze(1)
        x +=token_embedding_J_Beta.unsqueeze(1)
        
        if output_attentions:
            x.requires_grad_()    # used for attn_map output
            
        pos_emb_msg = get_final_pos_emb(self.pos_emb(x),enc_pos_mask)
        ...
        if output_attentions:
            ...
            x = self.performer(x, pos_emb = layer_pos_emb, output_attentions = output_attentions, mask=enc_self_attn_mask, **kwargs)
